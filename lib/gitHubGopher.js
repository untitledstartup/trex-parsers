(function(){
  ...
  GitHubGopher.prototype.contentCounter = null;
  ...
  GitHubGopher.prototype.stop = function stop() {
    this.queue.stop();
  };
  GitHubGopher.prototype.resume = function resume() {
    this.queue.resume();
  };
  GitHubGopher.prototype.end = function end() {
    this.queue.end();
  };
  ...
  GitHubGopher.prototype.onReposGetContent = function onReposGetContent(result) {
    if (result.content) {
      this.contentCounter++;
      if (this.contentCounter >= this.files.length) {
        var nextPage = this.page+1;
        Logger.info(">>> FETCHING PAGE: " + nextPage);
        this.fetchContentMatchingQuery(this.query, nextPage, this.perPage, this.sort, this.descending);
      }
    }
  };
  ...
  GitHubGopher.prototype._performGitHubRequest = function(apiPath, msg) {
    ...
    this.queue.push(function(cb) {
      // Logger.info(">>> API REQUEST: " + apiPath);

...
// Imports
  ...
  var TokenParser = null; // load before parsing, to avoid circular includes
...
// Languages
  ...
  var MarkupLanguages = ["django", "backbone", "express"];
...
// Genstrings
   ...
   *   
   */
  function Genstrings(tokenParser) {
    if (!TokenParser) {      
      TokenParser = require(Path.join(__dirname, "tokenParsers", "tokenParser.js"));
    }
    if (tokenParser instanceof TokenParser) {
  ...
  /**
   * Stores instanceof TokenParser used during instantiation, which is to be used
   * for parsing files, which overrides detection based on file extension...
   */
  Genstrings.prototype.tokenParser = null;
  ...
  Genstrings.prototype.parseFile = function parseFile(file) {
    ...
    var parser = (this.tokenParser) ? this.tokenParser :  TokenParser.parserForLanguage(lang);
    if (!parser) {
